...
- [ ] **2.11 Die Live-Demo entfällt – CTAs neu ordnen** (Briefing 0031, Entscheidung Stefan, 08.09.2026). Es gibt keinen Bereich, in dem ein Interessent selbstständig etwas ausprobiert, und es wird keinen geben; möglich ist nur ein **Termin für eine Live-Präsentation** – dafür steht bereits das Online-Erstgespräch. Die Mocks führen die Live-Demo an 27 Stellen, auf sechs Seiten als **primären grünen Button** mit dem Zusatz „ohne Anmeldung“ – ein Versprechen, das die Website nicht einlösen kann. **Regel:** Wo beide Aktionen angeboten wurden, bleibt genau eine – „Online-Erstgespräch vereinbaren / 30 Minuten persönlich per Zoom oder Teams“; war sie dort sekundär, wird sie primär. Betroffen: sechs Hero-Blöcke, acht `FooterClose`-Abschlüsse, zwei einzelne Verweise, die Kontaktseite (drei Wege werden zwei), das CTA-Ziel `livedemo` in `content/types.ts`/`lib/links.ts`/`config/site-structure.ts`, `NEXT_PUBLIC_LIVE_DEMO_URL` sowie das Sanity-Schema (`cta.target`, `siteSettings.liveDemoUrl`). **Die Mocks werden nicht nachgezogen** und sind an diesen Stellen überholt.
...
- [x] **3.4 Praxis-Seiten (der Blog)** (Briefing 0027). `/praxis` (Liste mit Themenfilter), `/praxis/thema/[slug]` (Rubrikseite) und `/praxis/[slug]` (Artikel mit Portable-Text-Renderer, Inhaltsverzeichnis, Autorenbox, „Passt dazu", CTA zum Erstgespräch). Die Route `/praxis` löst die Platzhalterseite aus 2.8 ab; sie bleibt vorerst `geplant`/`noindex`, bis Fred den ersten Artikel veröffentlicht hat (Frage 2 des Briefings). Im Repo steht **kein** Artikel-, Rubrik- oder Autorentext. **Vorab geklärt und gemessen** (docs/entscheidungen.md, 08.09.2026): `notFound()` liefert ohne JavaScript einen leeren Rumpf – gelöst über `proxy.ts`, der unbekannte `/praxis/…`-Adressen auf einen Pfad ohne Route umschreibt, sodass Nexts eigener 404 greift (Status 404 **und** voller Inhalt ohne JS). **Ebenfalls gemessen:** ein `loading.tsx` lässt Next streamen und macht die Seiten ohne JavaScript unlesbar – es entfällt deshalb auf allen drei Routen, `error.tsx` (Alert + Retry) bleibt. Typografie nach 2.2 (Artikel 18 px, max 70ch), Bilder mit LQIP-Blur und fester `aspect-ratio`, Leerzustand aus `lib/ui/messages.ts`. Abweichungen vom Mock: kein Newsletter-Block, Blättern per `?seite=` statt Nachladen, Themenfilter über eigene Adressen statt `?rubrik=` (docs/03, docs/04, docs/entscheidungen.md).
...
- [ ] **4.4 Erstgespräch-CTA.** Alle „Online-Erstgespräch vereinbaren"-Buttons lesen `NEXT_PUBLIC_BOOKING_URL`; Fallback auf Kontaktformular, wenn leer. Fred nennt den bestehenden Buchungsweg **[F]**. **Seit 2.11 ist das das einzige CTA-Ziel der Website** – die Live-Demo entfällt ersatzlos, `NEXT_PUBLIC_LIVE_DEMO_URL` gibt es nicht mehr. Termine laufen über cal.com (Entscheidung 07.09.2026).
