...
- [ ] **3.6 FAQ aus Sanity** (Briefing 0030). **Die sieben `faq`-Dokumente sind am 08.09.2026 angelegt und veröffentlicht** (topic `pakete`, `order` 1–7, feste IDs `faq-pakete-1` … `faq-pakete-7`) – über den Sanity-Connector statt im Studio, als bewusste Ausnahme von `CLAUDE.md` (Entscheidung Stefan, siehe `docs/entscheidungen.md`); der Wortlaut ist maschinell gegen `content/pakete.ts` geprüft und wortgleich. **Offen ist nur noch die Verdrahtung:** `/pakete` liest die Fragen aus Sanity statt aus dem Repo, Reihenfolge über `order`, Antwort als `simpleBlockContent`; `paketeFaq` fällt aus `content/pakete.ts` weg, ohne Rückfall. `FAQPage`-JSON-LD bleibt bei 6.4.
