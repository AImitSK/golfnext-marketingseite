...
- [ ] **3.9 Artikel-Teaser aus Sanity** (Briefing 0029). Die Teaser auf der Startseite (Abschnitt 8 „Praxis") und im Wissen-Slider auf `/ueber-golfnext` stehen noch als statische Daten im Repo – auf der Startseite mit **vollständigen Titeln, Autoren, Daten und Lesezeiten von Artikeln, die es nicht gibt** (aus Mock 3.1b übernommen). Beide Blöcke lesen künftig die neuesten Artikel aus Sanity und verlinken auf `/praxis/<slug>`; die Optik bleibt unverändert. **Der Wissen-Slider ist gemischt:** die zwei Karten „golfmanager · Fachartikel" sind Freds Fachzeitschriften-Beiträge und bleiben Platzhalter, bis Fred Titel, Teaser und Ausgabe liefert **[F]**. Ohne Artikel entfällt der Praxis-Abschnitt der Startseite vollständig.
