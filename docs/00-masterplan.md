...
- [x] **3.9 Artikel-Teaser aus Sanity** (Briefing 0029). Die Teaser auf der Startseite (Abschnitt 8 „Praxis") und im Wissen-Slider auf `/ueber-golfnext` kommen aus Sanity (`NEUESTE_POSTS_QUERY` mit `$anzahl`) und verlinken auf `/praxis/<slug>`; die Optik beider Blöcke bleibt, geteilt ist nur die Datenschicht. Die drei ausformulierten Teaser in `content/startseite.ts` (Titel, Autoren, Daten und Lesezeiten von Artikeln, die es nicht gibt) und die vier Platzhalterkarten in `content/ueber-golfnext.ts` sind **ersatzlos entfallen**, mit ihnen der Typ `PraxisArtikel` und das Feld `quelle`. **Kein Rubrik-Filter im Slider:** Seit Stefan am 08.09.2026 die Rubrik „Fachartikel" angelegt hat, sind Freds Fachzeitschriften-Beiträge normale Artikel; an Stelle der Quelle steht die Rubrik, Auswahl und Reihenfolge steuert Fred über das Veröffentlichungsdatum. Ohne Artikel entfallen **beide** Abschnitte vollständig, ohne Layoutlücke. **Die Lesezeit entfällt auf den Karten** – die Kartenabfrage trägt keinen Fließtext, und eine Zahl wird nicht geschätzt; im Slider steht an ihrer Stelle das Datum. Geprüft über zwei Testbestände und zwei Testserver (`pnpm build:e2e`, `scripts/build-e2e.mjs`).
